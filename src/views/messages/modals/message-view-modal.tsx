...
function MessageViewModal({ onClose, selectedId }: IMessageViewModal) {
  const toast = useToast();

  const [detailedMessage, setDetailedMessage] =
    useState<IMessageItemDetailed>();

  const fetchMessageById = async (id: string) => {
    try {
      const res: IGetMessageDetailedResponse =
        await MessagesServices.getInstance().getMessageById(id);
  ...
  useEffect(() => {
    selectedId && fetchMessageById(selectedId);
  }, []);
        ...
        {detailedMessage ? (
          <Flex>
            ...
            <Stack ml={8} p={3} spacing={2}>
              <Text fontSize="sm">
                {detailedMessage?.senderFirstname ?? noText}
              </Text>
              <Text fontSize="sm">
                {' '}
                {detailedMessage?.senderLastname ?? noText}
              </Text>
              <Text fontSize="sm">
                {' '}
                {detailedMessage?.senderPhone ?? noText}
              </Text>
              <Text fontSize="sm">
                {' '}
                {detailedMessage?.sendersGender ?? noText}
              </Text>
              <Text fontSize="sm"> {detailedMessage?.createdAt ?? noText}</Text>
              <Text fontSize="sm">
                {' '}
                {detailedMessage?.organizationName ?? noText}
              </Text>
              <Text fontSize="sm">
                {' '}
                {detailedMessage?.description ?? noText}
              </Text>
            </Stack>
          </Flex>
        ) : (
