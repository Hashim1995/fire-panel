...
function UserAddModal({ onClose, setRefreshComponent }: modalClose) {
  ...
  const fetchAllBranches = async () => {
    const res = await UsersServies.getInstance().getBranches();
    if (res?.succeeded) {
      setBranches(
        res?.datas?.map((item: IBranches) => ({
          label: item?.name,
  ...
  const onSubmit: SubmitHandler<IUserItem> = async (
    ...
    const payload = {
      ...
      birthday: null,
      apiLogin: 'ramiz',
      genderId: data?.genderId?.value ? Number(data?.genderId?.value) : null,
      roleId: data?.roleId?.value ? Number(data?.roleId?.value) : null,
      branchId: data?.branchId?.value ? Number(data?.branchId?.value) : null
    };
    try {
            ...
            <Stack spacing={4}>
              ...
              {watch('roleId')?.value === '2' && (
                <Controller
                  ...
                  render={({ field: { onChange, value } }) => (
                    <FormControl
                      isInvalid={Boolean(errors.branchId)}
                      id="branchId"
                      isRequired
                    >
                      <FormLabel fontSize="sm" mb={1}>
                      ...
                      <Tooltip
                        hasArrow
                        placement="top-end"
                        label={errors.branchId ? errors.branchId.message : ''}
                      >
                        <div>
                          <Select
                            onChange={onChange}
                            value={value}
                            options={branches}
                            placeholder={selectPlaceholderText('Filial')}
                          />
                        </div>
                      </Tooltip>
                    </FormControl>
